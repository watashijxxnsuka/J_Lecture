...
//		System.out.println(getMaxList(strList));
		...
		// Integer 클래스와 부모, 자식관계로 매핑되어 있지 않기 때문에 super 키워드로 
		// 하한제한을 건 매개변수로는 사용할 수 없다.
//		System.out.println(getAvgList(longList));
		
	}
	
	// 1. 타입매개변수를 지정하지 않고 바로 사용 가능한 제네릭 타입을 와일드카드<?>라고 한다.
	// 와일드카드는 제네릭클래스에서 사용할 수 없고 제네릭 메소드에서만 사용할 수 있다.
...
//	}
	...
	// 2. wildCard<?> 도 extends 로 상한제한을 걸 수 있다.
	...
	// 3. wildCard<?>는 super 키워드로 하한제한을 걸 수 있다.
	...
	// 4-2. 제네릭 타입매개변수는<T, K, V, E, ....>는 메소드의 정의부나 클래스의 정의부에 타입매개변수를 
	// 항상 지정해야하지만, 와일드카드<?>는 타입매개변수를 지정 안해도 된다.
	
	// 4-3. 제네릭 타입매개변수<T, K, V, E, ....>는 
	// 단독으로 사용가능(타입매개변수 타입의 변수선언, 리턴타입, 매개변수)
	// 와일드카드는 단독으로 사용할 수 없고, 제네릭 클래스의 서브클래스 타입으로만 지정 가능하다.
	// 와일드카드는 제네릭클래스의 확장성을 한단계 더 높여서 사용을 더 유연하게 해주는 기능을 한다.
	public static List<?> copyList(List<?> wildcardList) {
		List<Integer> returnList = new ArrayList<>();
		List<Map<String, String>> mapList = new ArrayList<>();
		
		// 와일드카드는 향상된 for 문도 사용 불가능
		for (int i = 0; i < wildcardList.size(); i++) {
